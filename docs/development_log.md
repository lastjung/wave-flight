...
## 최근 변경
- 테스트 관련 모듈/스크립트 제거 및 `node_modules` 정리
- 전자막(벽) 비활성화로 진행 방해 제거
- 안개(포그) 제거로 화면 막 느낌 해소
- 상하 조작 반전 해제(현재: W/↑ 위, S/↓ 아래)
- 탄환 타입 추가: NORMAL/PIERCE/BLAST/EMP, Q로 전환
- EMP 펄스 시각 효과 및 적 스턴 처리
- 장애물 파괴/점수 획득, 탄환 히트 이펙트 강화
